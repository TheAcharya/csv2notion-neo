...
def data_read(
    file_path: Path, fail_on_duplicate_columns: bool, args: Dict
) -> List[CSVRowType]:
    ...
    if "csv" in suffix:
        try:
            with open(file_path, "r", encoding="utf-8-sig") as csv_file:
                return _csv_read_rows(
                    csv_file, fail_on_duplicate_columns, args.delimiter
                )
        ...
        try:
            if args.hugging_face_token:
                if args.caption_column:
                    ...
                    raise Exception(
                        "ai token provided, please provide column maps to fill the ai content"
                    )
