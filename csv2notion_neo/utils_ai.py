...
class AI:
    ...
    def out(self, row) -> str:
        ...
        if "caption" in self.ai_data:
            ...
            try:
                row[data["caption_column"]] = self._img2caption(
                    data["hftoken"], str(data["image_path"]), data["model_url"]
                )
                return row
    ...
    def _img2caption(self, token: str, image_url: str, model_url: str) -> str:
        try:
            ...
            tqdm.write(
                f"Caption generated for image {filename} : {caption.json()[0]['generated_text']}"
            )
