...
def upload_filetype_official(
    ...
    if isinstance(filetype, Path):
        url, meta = upload_file_official(client, filetype)
    else:
        url = filetype
        meta = {"source": [[url]]}
...
def get_file_id_official(image_url: str) -> Optional[str]:
    """
...
def is_meta_different_official(
    ...
    if isinstance(image, Path) and image_url is not None and image_meta is not None:
        return _is_file_meta_different_official(image, image_url, image_meta)
...
def _is_file_meta_different_official(
    image: Path, 
    image_url: str, 
    image_meta: Meta
) -> bool:
    ...
    if image_meta.get("file_id") != get_file_id_official(image_url):
        return True
    
    return image_meta.get("sha256") != get_file_sha256(image)


def create_file_upload_official(
    client: NotionClientOfficial,
    filename: str,
...
def complete_file_upload_official(
    client: NotionClientOfficial,
    file_upload_id: str
) -> Dict[str, any]:
    """
...
def get_content_type_official(file_path: Path) -> str:
    """
...
def upload_file_with_retry_official(
    client: NotionClientOfficial,
    file_path: Path,
    max_retries: int = 3
) -> Tuple[str, Meta]:
    ...
    for attempt in range(max_retries):
        try:
            return upload_file_official(client, file_path)
        except Exception as e:
            last_error = e
...
def validate_file_for_upload_official(file_path: Path) -> None:
    """
...
def get_file_upload_status_official(
    client: NotionClientOfficial,
    file_upload_id: str
) -> Dict[str, any]:
    """
