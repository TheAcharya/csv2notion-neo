...
$TOOL_PATH --workspace "$NOTION_WORKSPACE" --token $NOTION_TOKEN --url $NOTION_URL --randomize-select-colors --image-column "Image Filename" --randomize-select-colors --image-column-keep --mandatory-column "Marker ID" --icon-column "Icon Image" --payload-key-column "Marker ID" --merge  --max-threads 5 --max-threads 1  --log "$UPLOAD_LOG" --verbose "$UPLOAD_PAYLOAD"